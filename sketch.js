let capture;
let graphics;

function setup() {
  // 建立全螢幕畫布，背景顏色為黑色
  createCanvas(windowWidth, windowHeight);
  background(0);

  // 初始化攝影機影像
  capture = createCapture(VIDEO);
  capture.size(windowWidth, windowHeight); // 設定影像寬高為全螢幕
  capture.hide(); // 隱藏原始的 HTML 視訊元素

  // 使用 createGraphics 產生一個與攝影機影像相同大小的畫布
  graphics = createGraphics(capture.width, capture.height);
  graphics.clear(); // 確保 graphics 初始為透明
}

function draw() {
  background(0); // 確保背景顏色維持為黑色

  // 水平翻轉畫布
  push();
  translate(width, 0); // 將畫布的原點移到右上角
  scale(-1, 1); // 水平翻轉畫布

  // 將攝影機影像顯示在畫布上
  image(capture, 0, 0, width, height);

  // 在 graphics 上繪製圓形濾鏡效果
  graphics.clear(); // 清除之前的繪製內容
  for (let x = 0; x < graphics.width; x += 20) {
    for (let y = 0; y < graphics.height; y += 20) {
      // 從攝影機影像中取樣顏色
      let col = capture.get(x, y);
      graphics.fill(red(col), green(col), blue(col), 150); // 設定顏色為半透明
      graphics.noStroke();
      graphics.ellipse(x + 10, y + 10, 15, 15); // 繪製寬高為 15 的圓形
    }
  }

  // 將 graphics 畫布顯示在攝影機影像的上方
  image(graphics, 0, 0, width, height);

  pop(); // 恢復畫布的原始狀態
}

function windowResized() {
  // 當視窗大小改變時，重新調整畫布大小
  resizeCanvas(windowWidth, windowHeight);
  capture.size(windowWidth, windowHeight); // 同步調整攝影機影像大小
}
